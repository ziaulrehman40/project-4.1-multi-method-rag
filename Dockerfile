...
CMD ["sh", "-c", "python manage.py migrate && python manage.py ensure_superuser && python manage.py ingest_docs && python manage.py build_graph && python manage.py build_trees && exec gunicorn config.wsgi:application --bind 0.0.0.0:${PORT:-8000}"]
